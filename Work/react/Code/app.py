...
def get_comment_count_api(url, api_key):
...
# 사이드바: API 키 입력 (보안을 위해 비밀번호 형태로 입력)
with st.sidebar:
    st.header("설정")
    api_key_input = st.text_input("유튜브 Data API 키 입력", type="password")
    st.info("팀원들에게 공유할 때는 API 키를 미리 알려주거나, 코드 내부 환경변수로 고정할 수도 있습니다.")
    ...
    try:
        ...
        if st.button("댓글 수 업데이트 시작", type="primary"):
            if not api_key_input:
                st.error("좌측 사이드바에 API 키를 입력해 주세요.")
            else:
                # 진행 상태 표시 준비
                ...
                for index, row in df.iterrows():
                    url = row[url_column]
                    count = get_comment_count_api(url, api_key_input)
